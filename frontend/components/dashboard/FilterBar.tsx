...
export const FilterBar = ({ makes, onFilterChange }: FilterBarProps) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <div
        className={`absolute -inset-0.5 bg-gradient-to-r from-brand-primary to-purple-600 rounded-2xl blur opacity-20 transition duration-500 ${isOpen ? "opacity-40" : "group-hover:opacity-30"}`}
      ></div>
      ...
      >
        <div className="flex items-center gap-3">
          <LucideFilter
            size={14}
            className={`transition-colors ${isOpen ? "text-brand-primary" : "text-slate-500"}`}
          />
        ...
        <LucideChevronDown
          size={14}
          className={`text-slate-500 transition-transform duration-300 ${isOpen ? "rotate-180 text-brand-primary" : ""}`}
        />
      </button>
